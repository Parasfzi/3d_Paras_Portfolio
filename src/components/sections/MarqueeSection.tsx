      ...
      <div className="flex flex-col gap-3">
        ...
        >
          {[...row1, ...row1, ...row1].map((src, i) => (
            <img key={`r1-${i}`} src={src} className="w-[420px] h-[270px] rounded-2xl object-cover flex-shrink-0" loading="lazy" alt="" />
          ))}
        </div>
        <div 
          className="flex gap-3 whitespace-nowrap" 
        ...
        >
          {[...row2, ...row2, ...row2].map((src, i) => (
            <img key={`r2-${i}`} src={src} className="w-[420px] h-[270px] rounded-2xl object-cover flex-shrink-0" loading="lazy" alt="" />
          ))}
        </div>
      </div>
    </section>
